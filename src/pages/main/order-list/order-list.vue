          ...
          <div class="order-list" v-if="list.data.length > 0">
            <div v-for="(item, itemIndex) in list.data" :key="itemIndex" @click="jumpDetail(item)" class="order-item">
              ...
              <div class="bot">
                ...
                <button v-if="!item.groupon.id && item.status!==0 && item.status!==3" :id="item.order_id + '&'+ item.shop_id " open-type="share" class="share-btn" @click.stop="">晒单</button>
              </div>
            </div>
          </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    onShareAppMessage(e) {
      ...
      const flag = Date.now()
      console.info(`${this.$routes.main.SHARE_ORDER}?id=${id}&shopId=${shopId}&flag=${flag}`)
      return {
